---
description: Create specs, enrich expected responses, or build flows. Usage: /rqb [scan|enrich|flow] [target]
---
...
```bash
rqb import project ${ARGUMENTS:-.}
```
If `✓ Found OpenAPI spec` → skip to post-scan. If stubs created → continue.

If the CLI suggests an export command (FastAPI, Spring Boot, etc.), run it then `rqb import openapi <file>`.
...
**Post-scan:** `rqb validate api-docs/` then `rqb index`.
...
2. Run `rqb_exec` with `infer_expected: true` if the server is up — use the live response.
...
```
Write to `api-docs/flows/<name>.md`. Then: `rqb validate api-docs/flows/<name>.md` and `rqb index`.

**To run:**
```json
{ "tool": "rqb_flow", "pipeline_path": "api-docs/flows/<name>.md", "env": "dev" }
```
