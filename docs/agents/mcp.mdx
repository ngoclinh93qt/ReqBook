---
title: "MCP server"
description: "Eight structured tools for Claude Code exec, flow, author, vars, search, history, session, and exec_batch. Compact output by default; verbose mode for debugging."
icon: "plug"
---

The MCP server exposes Trellis operations as structured JSON-RPC tools. Every tool returns compact JSON by default full request/response payloads only appear when you pass `verbose: true`. This keeps agent context lean.
...
| Tool | Purpose |
|---|---|
| `trellis_exec` | Execute one endpoint spec compact result by default |
| `trellis_flow` | Run a pipeline compact per-step summary by default |
| `trellis_author` | Create or update a spec (validates before writing, refuses silent overwrite) |
| `trellis_vars` | Show which variables a spec needs and which are resolved |
| `trellis_search` | Search specs by method, path, tag, or text no file reading required |
| `trellis_history` | Execution history and trend for a spec |
| `trellis_session` | Get/set session env + vars (defaults for all exec/flow calls) |
| `trellis_exec_batch` | Run multiple specs and return a summary table |

**Not in MCP use these directly:**

| Operation | How |
|---|---|
| Validate specs | `trellis validate <path>` |
| Start the browser UI | `trellis serve` |
| Import from cURL | `trellis import curl '...'` |

---

## Compact vs verbose output

By default, `trellis_exec` and `trellis_flow` return only what agents need to act: pass/fail, status code, diff, and error type. Full request and response bodies are omitted.
...
<Note>
 `trellis_author` validates the spec before touching the filesystem. If validation fails, nothing is written.
</Note>
...
After this, all `trellis_exec` / `trellis_flow` calls use `env: "staging"` and `authToken` by default.
...
Specs can include structured assertions in a `## Assertions` section. Trellis evaluates these after each execution and returns results in `assertion_results`.
...
Assertion results appear in `trellis_exec` output:
...
```text
1. trellis_search { "method": "POST", "tag": "orders" }
 → Finds api-docs/apis/orders/post-orders.md

2. trellis_vars { "spec_path": "api-docs/apis/orders/post-orders.md" }
 → ready: false authToken is missing

3. trellis_session { "action": "set", "env": "dev", "vars": { "authToken": "tok_abc" } }
 → Session set

4. trellis_exec { "spec_path": "api-docs/apis/orders/post-orders.md" }
 → { "passed": false, "error_type": "CONTRACT_MISMATCH", "hints": [...] }

5. trellis_exec { "spec_path": "...", "infer_expected": true }
 → { "inferred_expected": "HTTP/1.1 201 Created\n..." }

6. trellis_author { "spec_path": "...", "content": "...(with inferred expected)...", "overwrite": true }
 → Spec updated

7. trellis_exec_batch { "specs": ["...", "...", "..."] }
