...
The MCP server exposes MarkApiDown operations as structured JSON-RPC tools. Every tool returns compact JSON by default full request/response payloads only appear when you pass `verbose: true`. This keeps agent context lean.
...
| Tool | Purpose |
|---|---|
| `mad_exec` | Execute one endpoint spec compact result by default |
| `mad_flow` | Run a pipeline compact per-step summary by default |
| `mad_author` | Create or update a spec (validates before writing, refuses silent overwrite) |
| `mad_vars` | Show which variables a spec needs and which are resolved |
| `mad_search` | Search specs by method, path, tag, or text no file reading required |
| `mad_history` | Execution history and trend for a spec |
| `mad_session` | Get/set session env + vars (defaults for all exec/flow calls) |
| `mad_exec_batch` | Run multiple specs and return a summary table |

**Not in MCP use these directly:**

| Operation | How |
|---|---|
| Validate specs | `mad validate <path>` |
| Start the browser UI | `mad serve` |
| Import from cURL | `mad import curl '...'` |

---

## Compact vs verbose output

By default, `mad_exec` and `mad_flow` return only what agents need to act: pass/fail, status code, diff, and error type. Full request and response bodies are omitted.
...
<Note>
 `mad_author` validates the spec before touching the filesystem. If validation fails, nothing is written.
</Note>
...
After this, all `mad_exec` / `mad_flow` calls use `env: "staging"` and `authToken` by default.
...
Specs can include structured assertions in a `## Assertions` section. MarkApiDown evaluates these after each execution and returns results in `assertion_results`.
...
Assertion results appear in `mad_exec` output:
...
Specs are also accessible as MCP Resources under the `mad://spec/` URI scheme. Agents can browse and read spec content directly through the protocol without calling any tool.
...
```text
1. mad_search { "method": "POST", "tag": "orders" }
 → Finds api-docs/apis/orders/post-orders.md

2. mad_vars { "spec_path": "api-docs/apis/orders/post-orders.md" }
 → ready: false authToken is missing

3. mad_session { "action": "set", "env": "dev", "vars": { "authToken": "tok_abc" } }
 → Session set

4. mad_exec { "spec_path": "api-docs/apis/orders/post-orders.md" }
 → { "passed": false, "error_type": "CONTRACT_MISMATCH", "hints": [...] }

5. mad_exec { "spec_path": "...", "infer_expected": true }
 → { "inferred_expected": "HTTP/1.1 201 Created\n..." }

6. mad_author { "spec_path": "...", "content": "...(with inferred expected)...", "overwrite": true }
 → Spec updated

7. mad_exec_batch { "specs": ["...", "...", "..."] }
