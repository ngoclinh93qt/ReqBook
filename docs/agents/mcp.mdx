---
title: "MCP server"
description: "Nine structured tools for MCP-compatible coding agents: exec, flow, author, vars, search, context, history, session, and exec_batch. Compact output by default; verbose mode for debugging."
icon: "plug"
---

The MCP server exposes Reqbook operations as structured JSON-RPC tools for MCP-compatible coding agents. Every tool returns compact JSON by default full request/response payloads only appear when you pass `verbose: true`. This keeps agent context lean.
...
| `rqb_context` | Return compact deterministic API context for a target, flow, or changed specs |
...
Return compact deterministic API context for an endpoint, flow, or changed specs. This is the lowest-token way to give an agent relevant request/response contracts without asking it to read many files.
...
```text
# Reqbook Agent Context
Target: api-docs/apis/users/post-users.md
...
Suggested verify:
