...
<Tip>
 For token-sensitive agent runs, start with `rqb_context` using `mode: "surgical"` and an explicit `intent`. It returns the executable API contract before the agent decides whether backend source inspection is necessary.
</Tip>
