...
description: "Two /mad commands for Claude Code and Codex CLI — each handles a complex, multi-step task that benefits from LLM reasoning rather than a fixed script."
icon: "terminal"
---

## Design philosophy

Simple terminal operations — `mad init`, `mad serve`, `mad validate`, `mad exec` — work best when run directly. They are fast, composable, and don't need an agent in the loop.
...
/mad
/mad src/routes/
/mad enrich api-docs/apis/users/post-users.md
/mad flow checkout: login, add items, place order, verify
/mad api-docs/flows/checkout.md
```

### Scan mode

**What makes it intelligent:** The agent doesn't just run `mad import project` and stop. When automatic discovery produces stubs, the agent reads your handler source code — request/response types, DTOs, model classes — and produces specs with real field names and semantically accurate example values. A `user.email` field becomes `"user@example.com"`, not `"string"`.

What the agent does:
1. Tries `mad import project` first (OpenAPI or running server)
...
4. Creates specs via `mad_author` MCP tool (validates before writing, never overwrites)
5. Runs `mad validate api-docs/` then `mad index`
...
2. Runs `mad_exec` with `infer_expected: true` if the server is up — uses the live response
...
5. Validates with `mad validate`

What the agent does when running:
- Executes `mad_flow` MCP tool
...
```
/mad-debug GET /users/:id is returning 404
/mad-debug the checkout flow fails at step 3
/mad-debug api-docs/apis/orders/post-orders.md
```

### Single endpoint

What the agent does:
1. Locates the spec via `mad_search` MCP or `rg`
...
| `5` | Secret in spec | Move value to `.env.local` or `MAD_*` |
...
2. Runs the full flow via `mad_flow` MCP
...
|---|---|
| Initialize a new project | `mad init --name my-api --dev-url http://localhost:3000` |
| Start the web preview | `mad serve` |
| Start the mock server | `mad mock api-docs/` |
| Run one endpoint spec | `mad exec api-docs/apis/users/get-user.md` |
| Validate all specs | `mad validate api-docs/` |
| Register MCP server | `claude mcp add mad -- mad mcp` |
| Import a curl command | `mad import curl` |
