...
`mad request` lets you fire off any HTTP request from the terminal no spec file needed. It feels like `curl` but with automatic variable resolution and optional spec saving.

## Usage

```bash
mad request <METHOD> <URL> [options]
```
...
- **Outside a git repo**: the collection is at `~/.mad/workspace/default/api-docs/`. Unsaved requests go to `~/.mad/workspace/scratch/api-docs/`.
