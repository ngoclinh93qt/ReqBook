...
description: "Every MarkApiDown endpoint is a markdown file with a YAML frontmatter header, a Request block, and an Expected response block."
icon: "file-lines"
---
...
<CardGroup cols={3}>
 ...
 `mad exec` and `mad validate` read the same markdown file for automation.
 </Card>
</CardGroup>
...
<Note>
 Unknown frontmatter keys produce a warning and are ignored. This forward-compatible behavior means future MarkApiDown versions can introduce new fields without breaking existing spec files.
</Note>
...
`## Expected response` must contain exactly one `http` fenced code block. MarkApiDown diffs the actual response against this block after every execution.
...
<Note>
 MarkApiDown v1.0.0 executes only the single `## Expected response` block. Error responses are reference examples for humans, the web preview source view, and AI agents.
</Note>
...
`## Assertions` contains structured rules that MarkApiDown evaluates after each execution. Results appear in `mad_exec` output as `assertion_results`.
...
<Note>
 MarkApiDown does not execute `agent-task` code. The block is read by AI agents as instructions.
</Note>
...
`## Notes` is free-form markdown for team context: rate limits, edge cases, links to related endpoints, and migration notes. The MarkApiDown parser ignores this section entirely.
...
<Tabs>
 ...
 mad exec api-docs/apis/users/get-user-by-id.md --env=staging --var id=usr_456

 # Dry run print the resolved request without sending
 mad exec api-docs/apis/users/get-user-by-id.md --dry-run
 ```
 </Tab>
</Tabs>
