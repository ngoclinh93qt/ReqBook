...
`## Assertions` contains structured rules that Trellis evaluates after each execution. Results appear in `trellis_exec` output as `assertion_results`.
...
<Note>
 Trellis does not execute `agent-task` code. The block is read by AI agents as instructions.
</Note>
