...
{/* TODO: diagram showing pipeline flow: Step 1 captures userId → Step 2 injects userId → Step 3 injects authToken */}
...
<Tabs>
  <Tab title="Capture">
    ...
    Use JSONPath-style dot notation for JSON responses (`response.body.<field>`). Captured values have the highest variable priority and override all other sources for all later steps in the pipeline.
  </Tab>
  <Tab title="Inject">
    ...
    Injected variables are resolved from earlier `Capture` directives. If the named capture has not happened yet — because the producing step hasn't run or failed — Trellis returns an error before sending the request.
  </Tab>
  <Tab title="Assert">
...
<Tabs>
  ...
  <Tab title="Parallel">
    Set `parallel: true` in the frontmatter to allow Trellis to run independent steps concurrently.
    ...
    {/* TODO: diagram comparing sequential vs parallel step execution order */}
  </Tab>
</Tabs>
...
trellis flow api-docs/flows/user-onboarding.md --env=staging --var email=test@example.com
...
{/* TODO: screenshot of trellis flow console output showing step-by-step results with captured values */}
