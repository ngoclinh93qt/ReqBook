...
<CardGroup cols={2}>
 ...
 Nine structured tools for MCP-compatible agents — exec, flow, author, vars, search, context, history, session, and exec_batch. Compact output by default.
 </Card>
...
| Build compact API context | `rqb_context` MCP | Gives the agent a bounded contract pack instead of many source files |
...
Nine tools that return structured JSON or compact context for the agent to act on programmatically:
...
| `rqb_context` | Build bounded API context for a target, flow, or changed specs |
