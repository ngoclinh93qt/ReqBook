...
description: "Import existing API specs from Postman, Insomnia, or OpenAPI into Trellis with a single command."
icon: "right-left"
---

## General approach

The import commands convert foreign formats into Trellis markdown endpoint files. Structural information URLs, HTTP methods, headers, request bodies, status codes maps cleanly and is imported directly. Dynamic behavior pre-request scripts, JavaScript assertions, OAuth flows cannot be mechanically converted and is imported as `agent-task` blocks for manual review.

### Post-import checklist

<Steps>
 <Step title="Validate the imported specs">
 Run `trellis validate api-docs/` and fix any exit code `2` errors before doing anything else. Each error includes the file path, line number when known, and a suggested fix.
 ...
 If `trellis validate` exits with code `5`, a secret was imported into a versioned spec file. Move the value to `.env.local` or a `TRELLIS_*` environment variable, then re-run validation.
 ...
 Run `trellis exec` against your development environment on at least one imported endpoint to confirm the spec resolves and the response matches.

 ```bash
 trellis exec api-docs/apis/users/get-users.md --env=dev
 ```
 </Step>
 ...
 **6. Re-run validation and exec**
 ...
 | Postman | Trellis | Notes |
 |---|---|---|
 | Collection | `api-docs/` project | Name and description become project prose in `trellis.md`. |
 ...
 | Auth helper (bearer/basic) | `auth:` in frontmatter or `trellis.md` | Maps directly for `bearer` and `basic` types. |
 ...
 **What needs manual review after import**
 ...
 | Dynamic variables (`{{$timestamp}}`, `{{$randomEmail}}`) | No direct Trellis equivalent. Replace with `--var` flags, `env.md` values, or pipeline captures. |
 ...
 **5. Review agent-task blocks**
 ...
 <Tab title="Concept mapping">
 | OpenAPI | Trellis | Notes |
 |---|---|---|
 | `info.title` | `name` in `trellis.md` | |
 ...
 **What needs manual review after import**
 ...
 | JSON Schema validation | Schema constraints are not executable in Trellis v1.0.0. Added as `agent-task` assertions for manual review. |
 | OAuth 2.0 / OpenID Connect | Model as a Trellis pipeline that captures the token and injects it into subsequent steps. |
