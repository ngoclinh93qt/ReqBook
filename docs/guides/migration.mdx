...
description: "Import existing API specs from Postman, Insomnia, or OpenAPI into MarkApiDown with a single command."
icon: "right-left"
---

## General approach

The import commands convert foreign formats into MarkApiDown markdown endpoint files. Structural information URLs, HTTP methods, headers, request bodies, status codes maps cleanly and is imported directly. Dynamic behavior pre-request scripts, JavaScript assertions, OAuth flows cannot be mechanically converted and is imported as `agent-task` blocks for manual review.

### Post-import checklist

<Steps>
 <Step title="Validate the imported specs">
 Run `mad validate api-docs/` and fix any exit code `2` errors before doing anything else. Each error includes the file path, line number when known, and a suggested fix.
 ...
 If `mad validate` exits with code `5`, a secret was imported into a versioned spec file. Move the value to `.env.local` or a `MAD_*` environment variable, then re-run validation.
 ...
 Run `mad exec` against your development environment on at least one imported endpoint to confirm the spec resolves and the response matches.

 ```bash
 mad exec api-docs/apis/users/get-users.md --env=dev
 ```
 </Step>
 ...
 **6. Re-run validation and exec**
 ...
 | Postman | MarkApiDown | Notes |
 |---|---|---|
 | Collection | `api-docs/` project | Name and description become project prose in `mad.md`. |
 ...
 | Collection variables | `_shared/env.md` or `.env.local` | Non-secret values only in `env.md`. |
 | Environments | `_shared/env.md` sections | Each Postman environment becomes a `## <name>` heading. |
 | Auth helper (bearer/basic) | `auth:` in frontmatter or `mad.md` | Maps directly for `bearer` and `basic` types. |
 ...
 **What needs manual review after import**
 ...
 | Dynamic variables (`{{$timestamp}}`, `{{$randomEmail}}`) | No direct MarkApiDown equivalent. Replace with `--var` flags, `env.md` values, or pipeline captures. |
 ...
 | Environment secrets | Postman environments containing tokens must move to `.env.local` or `MAD_*` env vars. |
 | Chained requests | Use a MarkApiDown pipeline to chain requests and capture response values between steps. |
 </Tab>
</Tabs>
 ...
 **4. Move secrets and review agent-task blocks**
 ...
 | Environment (base + sub) | `_shared/env.md` sections | Each becomes a `## <name>` heading. |
 ...
 **What needs manual review after import**
 ...
 | Item | Reason |
 |---|---|
 | Template tags (`{% now %}`, `{% uuid %}`) | Insomnia-specific tags have no direct MarkApiDown equivalent. Replace with pipeline captures or `--var` flags. |
 | Plugin-based pre/post-request logic | Imported as `agent-task` instructions. |
 | Environment secrets | Must move to `.env.local` or `MAD_*` env vars. |
 | OAuth 2.0 flows | Multi-step auth flows should become a MarkApiDown pipeline that captures the token and injects it into subsequent steps. |
 </Tab>
</Tabs>
 ...
 **5. Review agent-task blocks**
 ...
 <Tab title="Concept mapping">
 | OpenAPI | MarkApiDown | Notes |
 |---|---|---|
 | `info.title` | `name` in `mad.md` | |
 ...
 | `components/schemas` | `## Notes` prose | Schema documentation is preserved as readable notes. |
 ...
 **What needs manual review after import**
 ...
 | JSON Schema validation | Schema constraints are not executable in the current MarkApiDown release. Added as `agent-task` assertions for manual review. |
 | OAuth 2.0 / OpenID Connect | Model as a MarkApiDown pipeline that captures the token and injects it into subsequent steps. |
