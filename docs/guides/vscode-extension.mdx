...
The Reqbook VS Code extension is the in-editor bridge for teams that write API specs directly in `api-docs/`. It keeps the workflow narrow: edit markdown in VS Code, call the `rqb` binary for validation and execution, and keep the browser UI available for larger visual workflows.

<Note>
 The extension requires the `rqb` binary. Install Reqbook first with Cargo, npm, Homebrew, or the shell installer, then make sure `rqb version` works in a terminal.
</Note>
...
- Endpoint specs inside `api-docs/` with `method:` and `path:` frontmatter show `Run Endpoint`.
- Flow and pipeline specs under `api-docs/flows/` or `api-docs/pipelines/` show `Run Flow`.
- Non-runnable collection docs such as `api-docs/reqbook.md`, `README.md`, and `_shared/env.md` do not show Run controls.
...
|---|---|---|
| `reqbook.rqbPath` | `rqb` | Path to the `rqb` binary. Use an absolute path if VS Code cannot find `rqb` on its process `PATH`. |
| `reqbook.env` | `dev` | Environment used for `Run Endpoint` and `Show Agent Context`. |
| `reqbook.apiDocsRoot` | empty | Optional `api-docs` root override. Relative paths resolve from the workspace root. |
