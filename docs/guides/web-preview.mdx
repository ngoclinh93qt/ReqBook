---
title: "Web preview"
description: "Use the MarkApiDown browser UI to browse, execute, tweak, edit, import, and connect markdown API specs."
icon: "desktop"
---

# Browser UI for markdown API specs.

`mad serve` starts a local browser workspace over the markdown files in `api-docs/`. It is designed to sit beside your editor and coding agent: agents can write files, you can inspect and run them visually, and everything stays in sync through the filesystem.
...
<Note>
 MarkApiDown binds to `127.0.0.1` by default. Use `--host=0.0.0.0` only when you intentionally want to share the preview on your local network.
</Note>
...
<CardGroup cols={2}>
 ...
 Paste a cURL command from DevTools and generate a MarkApiDown endpoint spec.
 </Card>
...
```bash
mad flow api-docs/flows/checkout.md --env=dev
```
...
Use `.env.local` or `MAD_*` for secrets. MarkApiDown masks auth values in UI responses and reports.
...
<Steps>
 ...
 Open `mad serve` and inspect the new files in the sidebar.
 </Step>
...
Mock mode reads the same `## Expected response` blocks that `mad mock` uses on the standalone mock server. Both features share the same source of truth.

<Tip>
 If your specs have empty `## Expected response` blocks (stub `{}`), run `/mad enrich` to enrich them with real field names, or execute against a live server once and copy the response body into the spec manually.
</Tip>
