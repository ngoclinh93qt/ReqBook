...
<Tip>
 If your specs have empty `## Expected response` blocks (stub `{}`), run `/trellis-sync` to enrich them with real field names, or execute against a live server once and copy the response body into the spec manually.
</Tip>
