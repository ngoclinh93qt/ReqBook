...
```bash
mad exec <file> [--env=<env>] [--output=<format>] [--var key=val]... [--dry-run] [--timeout=<ms>] [--strict-assertions]
```
...
```bash
mad flow <file> [--env=<env>] [--output=<format>] [--var key=val]... [--parallel] [--no-parallel] [--timeout=<ms>] [--strict-assertions]
```
...
mad check api-docs/ --report json
```
...
Print compact deterministic API context for coding agents.
