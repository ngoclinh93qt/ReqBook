...
Use `--mode surgical` for most Codex/Claude/Cursor tasks. It returns only the method/path, variables, request body fields, success response fields, assertions, and verify commands. Use `--mode schema` when another tool or agent needs machine-readable contract JSON.
...
rqb agent pack flow signup-login-profile --mode surgical --token-budget 1800
