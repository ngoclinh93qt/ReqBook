...
Reqbook returns stable exit codes you can reliably test for in CI scripts. See the [Exit codes reference](/docs/reference/exit-codes) for CI usage examples.
...
<Note>
When `--env=prod` is used in an interactive terminal, Reqbook prompts for confirmation before sending the request. Pass `--yes` (global flag) to skip the prompt in CI.
</Note>
...
```bash
rqb flow <file> [--env=<env>] [--output=<format>] [--var key=val]... [--parallel] [--no-parallel] [--timeout=<ms>] [--strict-assertions]
```
...
**After any import**: run `rqb validate api-docs/` and move any secrets to `.env.local`. See the [Migration guide](/docs/guides/migration) for per-tool concept mapping and post-import checklists.
...
rqb context --changed-from origin/main
```
...
| `rqb_validate` | Validate specs in a file or directory |
| `rqb_list_specs` | List all endpoint specs with method and path |
| `rqb_read_spec` | Read the full content of a spec file |
