...
description: "Project config (mad.md), environment config (env.md), auth modes, retry policy, and the web preview settings."
icon: "gear"
---

## Project config: `api-docs/mad.md`

`api-docs/mad.md` is the project configuration file. It uses YAML frontmatter and structured YAML code blocks inside named markdown sections.
...
| `version` | yes | integer | MarkApiDown spec format version. Must be `1` for the current spec format. |
...
The `## Web preview` code block controls `mad serve`. CLI flags (`--port`, `--host`) override these values.
...
the current MarkApiDown release does not execute plugins. The `## Plugins` section is reserved for future use. Keep it as an empty list or omit the section entirely.

### ## Notes section

Free-form prose, checklists, or team conventions. The MarkApiDown parser ignores this section entirely. Use it for anything that should live alongside the config but not affect execution.

---

## Auth modes

Set the auth mode in `mad.md`'s `## Defaults` section (project-wide) or in individual endpoint frontmatter (per-endpoint override).
...
| `custom` | The request block must include the `Authorization` header explicitly. MarkApiDown does not inject anything. |

<Tip>
Set a project-wide default in `mad.md`'s `## Defaults` block, then override per-endpoint with `auth:` in the endpoint's frontmatter. The endpoint value always wins.
</Tip>
...
Configure retries in `mad.md`'s `## Defaults` section or in individual endpoint frontmatter.
...
The environment name passed to `--env` must match one of these headings exactly. If the heading is missing, MarkApiDown exits with code `2`.

<Warning>
Do not put secrets in `env.md`. Tokens, passwords, and private keys must go in `.env.local` or `MAD_*` environment variables. The parser enforces this at validation time and exits with code `5` if a secret pattern is detected.
</Warning>
...
| 6 (lowest) | OS environment variables (`MAD_*`) | `MAD_USER_ID=99` |

OS environment variables are stripped of the `MAD_` prefix and converted to lower camel case:
