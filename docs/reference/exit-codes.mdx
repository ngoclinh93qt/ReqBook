---
title: "Exit codes"
description: "Reqbook returns stable exit codes so CI pipelines can act on pass, failure, invalid spec, or secret detection independently."
icon: "hash"
---
...
Reqbook found a pattern matching a secret (JWT token, hex API key, `sk_` prefixed key, etc.) in a versioned markdown file. The process exits immediately no request is sent, no spec is executed. Move the value to `.env.local` or a `RQB_*` environment variable.
