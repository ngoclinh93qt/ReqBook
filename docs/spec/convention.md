# Reqbook markdown convention

Reqbook is a markdown-native API spec system. A Reqbook project stores API documentation, executable requests, expected responses, pipelines, environment variables, and project configuration as markdown files. The same files are read by humans, the Rust engine, the web preview, CI, and AI coding agents.

This document is the canonical specification for the current Reqbook release. A developer who has never used Reqbook should be able to create a valid project from this document alone.

## Design goals

- Use markdown everywhere developers write or review API intent.
- Keep project configuration in `reqbook.md`, not a separate TOML, JSON, or YAML file.
...
A Reqbook project contains an `api-docs/` directory and a project-level `reqbook.md` file.
...
`api-docs/README.md` is generated by `rqb index`. Do not edit it by hand.
...
## Project config: `reqbook.md`

Project-level config lives in `api-docs/reqbook.md`. It uses the same parser pattern as endpoint files: required YAML frontmatter, markdown sections, and structured YAML code blocks.
...
| `version` | yes | integer | Reqbook spec format version. For this spec format this must be `1`. |
...
Sections are optional in the current release. If a section is missing, Reqbook uses built-in defaults. If a YAML code block contains an unknown key, Reqbook emits a warning and continues. This forward-compatible behavior allows future versions to add settings without breaking older clients.

`## Plugins` may contain an empty `plugins: []` block. the current Reqbook release does not execute plugins.
...
| `protocol` | yes | enum | `http`, `ws`, or `sse`. the current Reqbook release executes only `http`. |
...
If the URL is relative, Reqbook resolves it against the selected environment's `baseUrl`.
...
the current Reqbook release does not execute `## Error responses`; only `## Expected response` is compared during `rqb exec`. Error responses are reference examples for humans, web source view, and AI agents.

### Assertions block

The optional `## Assertions` section contains structured rules evaluated after each execution. Results appear in CLI output and `rqb_exec` MCP responses as `assertion_results`.
...
the current Reqbook release does not execute arbitrary code from `agent-task` blocks. Agents may use the tasks as instructions.
...
Reqbook uses `{{name}}` syntax in markdown content, request blocks, expected response blocks, pipeline definitions, and selected structured config values.
...
5. `.env.local`.
6. OS environment variables prefixed `RQB_`.
...
Only variables prefixed with `RQB_` are considered. The prefix is stripped and the remaining name is converted to lower camel case.
...
If a variable is referenced and not found, Reqbook returns a validation error before executing the request.
...
If resolving `{{baseUrl}}` produces another unresolved `{{apiHost}}`, Reqbook returns an error instead of performing a second pass. This prevents surprising precedence behavior and secret leakage.
...
Reqbook validates before execution.
...
If an endpoint declares `env: [dev, staging]` and the user runs `--env=prod`, Reqbook returns an invalid spec error with a suggested fix.
...
CI should pass secrets through environment variables such as `RQB_AUTH_TOKEN`.

### Output masking

Reqbook masks auth values in CLI output, logs, reports, browser responses, and execution history.
...
Reqbook commands use stable exit codes:
...
Reqbook imports Postman collections into markdown endpoint files. The result should be reviewed by a human because Postman scripts can contain arbitrary JavaScript and environment behavior that does not map perfectly to markdown.

### Concept mapping

| Postman concept | Reqbook concept | Notes |
| --- | --- | --- |
...
3. Run `rqb import postman collection.json`.
4. Run `rqb validate api-docs/`.
5. Move imported secrets to `.env.local` or `RQB_*` variables.
6. Run `rqb index`.
7. Review imported `agent-task` blocks for manual follow-up.

### Known migration edge cases

Postman pre-request scripts are not executed by Reqbook. They are imported as human-readable tasks.

Dynamic Postman variables such as `{{$timestamp}}` require explicit Reqbook variables or pipeline captures.

Postman tests that depend on JavaScript side effects are imported as `agent-task` instructions.

Postman environments containing tokens are rejected by Reqbook if imported into `_shared/env.md`; move them to `.env.local`.
...
Small API used to demonstrate Reqbook.
