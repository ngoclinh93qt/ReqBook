...
const version = env("RQB_VERSION", "MAD_VERSION") || require("./package.json").version;
const repo = env("RQB_REPO", "MAD_REPO") || "ngoclinh93qt/ReqBook";
const cacheRoot = env("RQB_CACHE_DIR", "MAD_CACHE_DIR") || path.join(os.homedir(), ".cache", "reqbook");
...
function binaryName() {
  return os.platform() === "win32" ? "rqb.exe" : "rqb";
}
...
async function ensureBinary() {
  const binaryOverride = env("RQB_BINARY", "MAD_BINARY");
  if (binaryOverride) {
  ...
  const archive = `rqb-${triple}${extension()}`;
