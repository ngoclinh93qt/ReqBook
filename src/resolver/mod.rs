...
/// Resolution context with Trellis priority ordering.
...
pub enum ResolveError {
    ...
    #[error("unresolved variable \"{name}\"\nFix: define {name} in .env.local, pass --var {name}=..., or set TRELLIS_{env_name}.")]
    MissingVariable {
        /// Variable name.
        name: String,
    ...
    #[error("nested variable remained after resolution: {name}\nFix: define final values directly; Trellis v1.0 does not recursively resolve variables.")]
    NestedVariable {
        /// Variable name.
        name: String,
    },
